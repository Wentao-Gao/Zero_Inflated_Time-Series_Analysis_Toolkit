...
[🇨🇳 中文说明](README_CN.md) | [🇺🇸 English](README.md) | [📚 文档](docs/) | [🔬 示例](examples/)

---

*✨ 从数据预处理到模型训练评估 - 您需要的零膨胀时间序列分析一站式解决方案！ ✨*
...
### 📈 **统计模型**
- 🎲 **ZIP** *(零膨胀泊松)*: 零膨胀泊松回归
- 🎯 **ZINB** *(零膨胀负二项)*: 零膨胀负二项回归
- 🌊 **Tweedie GLM**: Tweedie广义线性模型
- 🚪 **Hurdle Model**: 跨栏模型

### 🧠 **深度学习模型**
- 🔗 **ZIP-RNN**: 结合RNN和零膨胀泊松分布的神经网络
- 🌿 **双分支网络**: 分别建模零值和非零值的双分支网络
- ⚖️ **加权损失Transformer**: 带加权损失的Transformer
- ⚡ **增强Tweedie Transformer**: 增强的Tweedie Transformer
...
#### 3️⃣ **Pandas DataFrame格式**
```python
data = pd.DataFrame({
    'timestamp': pd.date_range('2023-01-01', periods=100, freq='H'),
    'value': [...],        # 🎯 主要的时间序列值（必需）
    'temperature': [...],  # 🌡️ 额外特征1（可选）
    'humidity': [...],     # 💧 额外特征2（可选）
    'is_weekend': [...]    # 📅 额外特征3（可选）
})
```
...
<table>
<tr>
<td><strong>✅ 必需要求</strong></td>
<td>
• 🔢 数值型时间序列数据（float或int）<br>
• ➕ 非负数值（零膨胀数据通常是计数或度量数据）<br>
• 🚫 无缺失值（NaN）或无穷值<br>
• 📊 至少50个观测点（建议100+用于可靠建模）
</td>
</tr>
<tr>
<td><strong>🔧 可选要素</strong></td>
<td>
• 📅 时间戳列或索引<br>
• 🔧 额外的特征列<br>
• ⏰ 规则的时间间隔
...
│   │   ├── ⚖️ weighted_loss_transformer.py # ⚖️ 加权Transformer
...
- 🙏 感谢 **scikit-learn** 团队提供的机器学习基础设施
- 🙏 感谢 **PyTorch** 团队提供的深度学习框架
- 🙏 感谢所有为 **零膨胀建模** 研究做出贡献的研究人员

---

## 📞 联系方式与支持

🚀 **快速开始**: [快速开始指南](docs/quickstart.md)  
📚 **完整文档**: [详细文档](docs/)  
🐛 **问题反馈**: [创建Issue](issues/)  
💬 **社区讨论**: [参与讨论](discussions/)  
