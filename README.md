...
A comprehensive toolkit for zero-inflated time series analysis, providing complete solutions from data preprocessing to model training and evaluation. This toolkit is specifically designed for handling time series data with excessive zeros, which is common in many real-world applications.
...
- **Specialized Preprocessing**: Preprocessing tools specifically for zero-inflated data
...
- **Weighted Loss Transformer**: Transformer with weighted loss
...
data = pd.DataFrame({
    ...
    'temperature': [...],  # Additional feature 1 (optional)
...
✅ **Required**:
- Numeric time series data (float or int)
- Non-negative values (zero-inflated data is typically count or measurement data)
...
✅ **Zero Value Handling**:
- Zero values are expected and meaningful
- Zero ratio can range from 5% to 80%
...
- **Ad Clicks**: Click rate prediction
...
│   │   ├── weighted_loss_transformer.py # Weighted Transformer
